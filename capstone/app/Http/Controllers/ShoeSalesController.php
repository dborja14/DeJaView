...
class ShoeSalesController extends Controller
{
    ...
    public function getMonthlyComparison()
    {
        ...
        $revenues = DB::table('orders')
            ->select(
                DB::raw('DATE_FORMAT(updated_at, "%M") as month'), // Format as "Month Year"
                DB::raw('SUM(totalPrice) as total_revenue')
            )
            ->where('paymentStatus', 'Approved')
            ->whereYear('updated_at', $currentYear) // Filter for the current year
            ->groupBy('month')
            ->orderBy('updated_at') // Ordering by date rather than formatted month
            ->get();

        $monthlyComparison = [];
        $previousRevenue = null;

        foreach ($revenues as $current) {
            if ($previousRevenue !== null) {
                ...
                $monthlyComparison[] = [
                    ...
                    'percentage_change' => $difference / $previousRevenue->total_revenue * 100
                ];
            }
